...
pub fn NewStruct(comptime Server: type) type {
    ...
    return struct {
        ...
        pub fn nothing(self: *@This()) !void {
            const msg = NewStructAsmParamUnion{ .nothing = .{} };
            try self.channel.send(msg);
        }
        pub fn something(self: *@This(), x0: i32, x1: f32) !void {
            const msg = NewStructAsmParamUnion{ .something = .{ x0, x1 } };
            try self.channel.send(msg);
        }
        pub fn selfFunction(self: *@This(), x0: *src.example.MyStruct) !void {
            const msg = NewStructAsmParamUnion{ .selfFunction = .{x0} };
            try self.channel.send(msg);
        }
        pub fn kkkk(self: *@This(), x0: *src.example.MyStruct) !void {
            const msg = NewStructAsmParamUnion{ .kkkk = .{x0} };
            try self.channel.send(msg);
        }
    };
}

pub const NewStructAsmTag = enum { nothing, something, selfFunction, kkkk };

pub const NewStructAsmParamUnion = union(NewStructAsmTag) { nothing: std.meta.tuple(&.{}), something: std.meta.tuple(&.{ i32, f32 }), selfFunction: std.meta.tuple(&.{*src.example.MyStruct}), kkkk: std.meta.tuple(&.{*src.example.MyStruct}) };

pub const NewStructAsmReturnUnion = union(NewStructAsmTag) { nothing: f32, something: void, selfFunction: void, kkkk: void };
