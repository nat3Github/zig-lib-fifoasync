...
pub fn code_gen(
    ...
    const parts = struct {
        ...
        const TNameParamUnion = "Args";
        const TNameReturnUnion = "Ret";
        const decls = zmeta.struct_decl_names(T);
        const comptime_decl_check = zfmt.fHasDeclCompileCheck("Channel", "send", "the server type is missing the send method");
        ...
        fn generate() [:0]const u8 {
            ...
            const sfn_body_fmt =
                ...
                \\}};
            ;
            const sfn_args = "comptime Channel: type";
            const sfn_return = "type";
            const sfn_body = comptime comptimePrint(sfn_body_fmt, .{ comptime_decl_check, msg_tag_enum, msg_union_fnargs(), msg_union_fnreturn(), message_handler(), delegator_fn_str() });
            const generic_fn = comptime zfmt.fFn(name, sfn_args, sfn_return, sfn_body);
            return comptime join("\n\n", &.{ "/// Autogenerated, will refresh if underlying struct changes and build.zig if setup in the build script\n/// changes to this file will likely be overwritten", zfmt.fImport("std", "std"), imports, generic_fn });
        }
    };
